...
try { await access('.env'); console.log('保留现有 .env'); }
  ...
  env = env.replace(/^API_KEY=$/m, `API_KEY=${randomBytes(24).toString('hex')}`)
    .replace(/^SESSION_KEY=$/m, `SESSION_KEY=${randomBytes(32).toString('base64')}`)
    .replace(/^DATABASE_URL=.*$/m, `DATABASE_URL=file:${resolve('data/bidcenter.db')}`);
  await writeFile('.env', env, { mode: 0o600 });
  console.log('已创建 .env（管理密钥与会话密钥已随机生成，请勿提交）');
}
