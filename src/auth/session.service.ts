...
export type State = Awaited<ReturnType<BrowserContext['storageState']>> & { userAgent?: string };
...
export function sessionCredentials(state: Pick<State, 'cookies'>) {
  ...
  if (!cookie) throw new SiteError('LOGIN_REQUIRED', '尚未登录采招网，请在登录窗口完成登录');
...
export class SessionService implements OnApplicationShutdown {
  ...
  async credentials(context?: APIRequestContext) {
    ...
    return sessionCredentials(state);
  }
